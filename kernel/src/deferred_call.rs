...
impl DeferredCall {
    ...
    pub fn has_tasks() -> bool {
        let closure = |defcall_state: &mut ThreadLocalDeferredCallState| defcall_state.bitmask != 0;
    ...
    pub fn verify_setup() {
        let closure = |defcall_state: &mut ThreadLocalDeferredCallState| {
            ...
            let num_registered_calls = defcall_state
                .defcalls
                .iter()
                .filter(|opt| opt.is_some())
                .count();
            let num_deferred_call_slots = defcall_state.defcalls.len();
            (
                num_deferred_calls,
                num_registered_calls,
                num_deferred_call_slots,
            )
        };
