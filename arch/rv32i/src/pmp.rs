...
pub mod test {
    ...
    #[test]
    pub fn test_app_memory_alloc() {
        // TODO! implement more test cases, such as:
        //
        // - Try to update the app memory break with an invalid pointer below
        //   its allocation's start address.
    }
}
...
pub mod kernel_protection_mml_epmp {
    ...
    /// |       |                                         |       |   |       |
    /// | 4 ... | /                                     \ |       |   |       |
    /// | n - 6 | \ Userspace TOR region #x             / |       |   |       |
    /// |       |                                         |       |   |       |
    /// | n - 5 | --------------------------------------- | OFF   | X | ----- |
    /// | n - 4 | Kernel .text section                    | TOR   | X | R/X   |
    /// |       |                                         |       |   |       |
        ...
        for KernelProtectionMMLEPMP<AVAILABLE_ENTRIES, MPU_REGIONS>
    {
        ...
        fn configure_pmp(
                ...
                .enumerate()
            {
                ...
                if region.0 != TORUserPMPCFG::OFF {
                    csr::CSR.pmpaddr_set(i * 2 + 0, (region.1 as usize).overflowing_shr(2).0);
                    csr::CSR.pmpaddr_set(i * 2 + 1, (region.2 as usize).overflowing_shr(2).0);
                }
        ...
        fn disable_user_pmp(&self) {
            ...
            let mut user_region_pmpcfg_octet_pairs = 0..MPU_REGIONS;
            ...
            for (i, shadowed_pmpcfg) in self.shadow_user_pmpcfgs.iter().enumerate() {
                let (start_pmpaddr_label, startaddr_pmpaddr, endaddr, mode) =
                    if shadowed_pmpcfg.get() == TORUserPMPCFG::OFF {
                        ("pmpaddr", csr::CSR.pmpaddr_get(i * 2), 0, "OFF")
                    } else {
                        (
                            "  start",
                            csr::CSR.pmpaddr_get(i * 2).overflowing_shl(2).0,
                            csr::CSR.pmpaddr_get(i * 2 + 1).overflowing_shl(2).0 | 0b11,
                            "TOR",
                        )
                    };
