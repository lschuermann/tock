...
extern "C" {
    ...
    /// code we need to. If the trap happens while and application was
    /// executing, we have to save the application state and then resume the
    /// `switch_to()` function to correctly return back to the kernel.
    ///
    /// We implement this distinction through a branch on the value of the
    /// `mscratch` CSR value. If, at the time the trap was taken, it contains
    /// `0`, we assume that the target is currently executing kernel code.
    ///
    /// If it contains any other value, we interpret it to be a memory address
    /// pointing to a particular structure:
    ///
    /// ```
    /// mscratch           0               1               2               3
    ///  \->|--------------------------------------------------------------|
    ...
    /// traps should be handled when they occur during process execution.
    /// This global trap handler behavior is well defined. It will:
    ///
    ...
    /// before contiuing execution at the address loaded into the mscratch CSR
    ...
    ///
    /// We explicitly use callee-saved over temporary registers, as this makes
    /// it easier to call other functions as part of the trap, handler, such as
    /// to disable interrupts. This global trap handler saves the previous
    /// values of these registers anyways, and places new, important values into
    /// them (the previous mscratch value and the trap handler address). These
    /// values are likely desired to not be clobbered by further function calls
    /// in the trap handler.
    ///
