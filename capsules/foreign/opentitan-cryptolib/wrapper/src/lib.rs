...
> {
    rt: RT,
    rt_scopes: RefCell<(
        encapfn::types::AllocScope<RT::AllocTracker, ID>,
        encapfn::types::AccessScope<ID>,
    )>,
...
impl<
        ...
        ABI: encapfn::abi::Encapfn4WRegABI,
        RT: encapfn::EncapfnRt<ID = ID, TargetABI = ABI>,
    > CryptolibHmac<'client, ID, ABI, RT>
{
    pub fn new(
        rt: RT,
        alloc_scope: encapfn::types::AllocScope<RT::AllocTracker, ID>,
        access_scope: encapfn::types::AccessScope<ID>,
    ) -> Self {
        CryptolibHmac {
            rt,
            rt_scopes: RefCell::new((alloc_scope, access_scope)),
    ...
    fn with_hmac_context<R>(
        &self,
        fun: impl FnOnce(
            encapfn::types::EFPtr<wrapped::HmacContext>,
            &mut encapfn::types::AllocScope<RT::AllocTracker, ID>,
            &mut encapfn::types::AccessScope<ID>,
        ) -> R,
    ) -> Result<R, ErrorCode> {
        ...
        let (ref mut alloc_scope, ref mut access_scope) = *self
            .rt_scopes
            .try_borrow_mut()
        ...
        self.cryptolib_hmac_context
            .map(|hmac_context_rust| {
                self.rt.allocate_stacked_t::<wrapped::HmacContext, _, _>(
                    alloc_scope,
                    |hmac_context_alloc, alloc_scope| {
                        use core::ops::Deref;

                        let hmac_context_ref = hmac_context_alloc.into_ref(&alloc_scope);
                        hmac_context_ref.write(hmac_context_rust.clone(), access_scope);
                        let hmac_context_ptr = hmac_context_ref.as_ptr();

                        let res = fun(hmac_context_ref.as_ptr(), alloc_scope, access_scope);

                        let hmac_context_ref_val = hmac_context_ptr
                            .upgrade_mut(&alloc_scope)
                            .unwrap()
