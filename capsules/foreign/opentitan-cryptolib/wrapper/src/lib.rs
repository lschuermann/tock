...
impl<'client, 'chip, C: Chip> CryptolibHmac<'client, 'chip, C> {
    ...
    fn with_hmac_context<'alloc, 'access, R>(
        &self,
        fun: impl FnOnce(encapfn::types::EFMutPtr<wrapped::HmacContext>, &mut encapfn::types::AllocScope, &mut encapfn::types::AccessScope) -> R,
    ) -> Result<R, ErrorCode> {
	...
	// context into that allocation:
        self.cryptolib_hmac_context
            .map(|hmac_context_rust| {
                self.rt
                    .allocate_stacked_t::<wrapped::HmacContext, _, _>(
			&mut alloc_scope,
			    ...
			    hmac_context_ref.write(hmac_context_rust.clone(), &mut access_scope);
                            ...
                            let res =
				fun(hmac_context_ref.into_ptr(), &mut alloc_scope, &mut access_scope);

			    let hmac_context_ref_val = hmac_context_ref.validate_ref(&access_scope).unwrap();
                            // {
