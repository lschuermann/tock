...
impl<'a, A: Alarm<'a>> AlarmDriver<'a, A> {
    ...
    fn process_rearm_or_callback(&self) {
        // Check whether there are any armed alarms. If there are none, we don't
        // need to iterate and can exit early:
    ...
    fn rearm_u32_left_justified_expiration(
        ...
        // First, increment the counter of pending alarms if the previous
        // expiration state was disabled:
        if expiration.is_none() {
            num_armed.set(num_armed.get() + 1)
        }
...
mod test {
    ...
    impl<'a, T: Ticks, F: Frequency> MockAlarm<'a, T, F> {
        pub fn new(initial_ticks: T) -> Self {
            MockAlarm {
                current_ticks: Cell::new(initial_ticks),
    ...
    fn test_earliest_alarm_multiple_expired() {
        let mut exp_list: [Cell<bool>; 7] = Default::default();
    ...
    fn test_earliest_alarm_expired_stop() {
        let mut exp_list: [Cell<bool>; 4] = Default::default();

        let exp_handler = |exp, id: &usize| -> Option<&'static str> {
            exp_list[*id].set(true);
